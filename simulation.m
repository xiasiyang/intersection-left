           ...
           if(i==1||i==2||i==11||i==12)
            if(i==1)   %第一阶段P1,P2可能左转，P1
                ...
                if(s(i,j,k)<(-d1))   %转弯前
                    ...
                    if(s(i,j,k)<(-d1+0.5*pi*R1))  %转弯中
                        o(i,j,k)=o(i,j,k-1)+(s(i,j,k)-s(i,j,k-1))/R1;
                        x(i,j,k)=-d1+R1*sin(o(i,j,k));
                        y(i,j,k)=2.5*d2+R1-R1*cos(o(i,j,k));
            ...
            elseif(i==12)   %P2
               ...
               if(i==3||i==5||i==13||i==15||i==17)  %其他车道的第一个车队
                   V(i,j,k)=V(i,j,k-1)+a(i,j,k-1)*t0;
                   s(i,j,k)=s(i,j,k-1)+V(i,j,k-1)*t0+0.5*a(i,j,k-1)*(t0)^2;
                   if(j==1)   %头车
                       if(V(i,j,k)>0)
               ...
               elseif(i==4||i==6||i==14||i==16||i==18)   %其他车道第二个车队
                   if(j==1)
                       V(i,j,k)=V(i,j,k-1)+a(i,j,k-1)*t0;
                       s(i,j,k)=s(i,j,k-1)+V(i,j,k-1)*t0+0.5*a(i,j,k-1)*(t0)^2;
                   ...
                   x(i,j,k)=x(i,j,k-1)+abs(s(i,j,k)-s(i,j,k-1))*cos(o(i,j,k));
                   y(i,j,k)=y(i,j,k-1)+abs(s(i,j,k)-s(i,j,k-1))*sin(o(i,j,k));
               elseif(i==7||i==9||i==19||i==21||i==23)  %其他车道的第一个车队
                   V(i,j,k)=V(i,j,k-1)+a(i,j,k-1)*t0;
                   s(i,j,k)=s(i,j,k-1)+V(i,j,k-1)*t0+0.5*a(i,j,k-1)*(t0)^2;
                   if(j==1)   %头车
                       if(V(i,j,k)<0)
                   ...
                   x(i,j,k)=x(i,j,k-1)+abs(s(i,j,k)-s(i,j,k-1))*cos(o(i,j,k));
                   y(i,j,k)=y(i,j,k-1)+abs(s(i,j,k)-s(i,j,k-1))*sin(o(i,j,k));
               elseif(i==8||i==10||i==20||i==22||i==24)   %其他车道第二个车队
                   if(j==1)
                       V(i,j,k)=V(i,j,k-1)+a(i,j,k-1)*t0;
                       s(i,j,k)=s(i,j,k-1)+V(i,j,k-1)*t0+0.5*a(i,j,k-1)*(t0)^2;
                       if(abs(s(i,j,k)-s(i-1,Mv,k)-L0)>=se)  %车队外间距大于等于se时，与ve比较决定加速度
...
Start = 0;      
ArrivalTime = 1:Ncs(55,tt);       %新数据包到无线节点发送缓冲区的时间
PacketLength = 1:Ncs(55,tt);     %到无线节点发送缓冲区的新数据包大小
HasPacket = zeros(1,Ncs(55,tt));
CW = 1:Ncs(55,tt);
BackoffTimer = 1:Ncs(55,tt);
PacketBuff = zeros(Ncs(55,tt),5001); %无线节点发送缓冲区各数据包大小矩阵
CollitionStations = zeros(1,Ncs(55,tt)+1);  %冲突站点号向量
SlotTime = 13*10^(-6);      %时隙间隔=50us
TotalTime = floor(TotalTimeInSeconds/SlotTime);  %总仿真时间，单位：slot
CurBufferSize = zeros(1,Ncs(55,tt)); %当前发送帧长
...
for t = 1:TotalTime
    ...
    if Start == 1%信道改为忙碌
        ...
        Start=0;%信道恢复空闲        
        CollitionStations = zeros(1,Ncs(55,tt)+1);
    end
end
...
PK_sim(1,tt)=(1-tao_sim(1,tt))^(Ncs(55,tt)-1);
...
Start_1 = 0;      
ArrivalTime_1 = 1:Ncs(55,tt);       %新数据包到无线节点发送缓冲区的时间
PacketLength_1 = 1:Ncs(55,tt);     %到无线节点发送缓冲区的新数据包大小
HasPacket_1 = zeros(1,Ncs(55,tt));
CW_1 = 1:Ncs(55,tt);
BackoffTimer_1 = 1:Ncs(55,tt);
cc = 1:Ncs(55,tt);
PacketBuff_1 = zeros(Ncs(55,tt),5001); %无线节点发送缓冲区各数据包大小矩阵
CollitionStations_1 = zeros(1,Ncs(55,tt)+1);  %冲突站点号向量
SlotTime_1 = 13*10^(-6);      %时隙间隔=50us
TotalTime_1 = floor(TotalTimeInSeconds_1/SlotTime_1);  %总仿真时间，单位：slot
CurBufferSize_1 = zeros(1,Ncs(55,tt)); %当前发送帧长
...
for t = 1:TotalTime_1%总仿真时隙
    for i = 1:Ncs(55,tt)%%%当前时隙下，目标范围内车辆数
    %更新发送缓冲区
    ...
    if Start_1 == 1%%%%由于范围内节点发包，信道改为忙碌
        ...
        Start_1=0;%信道恢复空闲         
        CollitionStations_1 = zeros(1,Ncs(55,tt)+1);
    end 
end
...
PK_sim1(1,tt)=(1-tao_sim1(1,tt))^(Ncs(55,tt)-1);
...
PK_sim2(1,tt)=(1-tao_sim2(1,tt))^(Ncs(55,tt)-1);
...
PK_sim3(1,tt)=(1-tao_sim3(1,tt))^(Ncs(55,tt)-1);
